...
func (c *createClone) run(cmd *cobra.Command, _ []string) error {
	...
	vmCloneObj, err := c.newClone()
	if err != nil {
		return err
	}

	err = c.applyFlags(cmd, &vmCloneObj.Spec)
	if err != nil {
		return err
	}

	if vmCloneObj.Name == "" {
		vmCloneObj.Name = "clone-" + rand.String(cloneRandomSuffixLength)
	}

	cloneBytes, err := yaml.Marshal(vmCloneObj)
	if err != nil {
		return err
	}
